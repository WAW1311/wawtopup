...
@section('howto')
          <p>Cara Topup Call of Duty Mobile<br>
...
@section('datagame')
          ...
          <button type="button" class="btn btn-primary" onclick="CheckUsername('CALL_OF_DUTY')">Cek username</button>
@endsection
@section('items')
        <div class="row row-cols-3 row-cols-md-3 g-3">
          ...
          @foreach ($product as $item)
            @if ($item['game'] == "Call of Duty Mobile")
              @php
                $price = app('App\Http\Controllers\handlercontroller')->profit($item['price']['basic']);
                $found = true;
              @endphp
                <div class="col">
                  <div id="selectproduct" class="card border border-primary card-hover" style=" cursor: pointer;" onclick="handleCardClick(this)">
                    <div class="card-body" style="padding: 0.5rem;position:relative;">
                      <div class="card-items">
                        <center><div class="img-item">
                          <img class="img-fluid rounded" src="{{ asset('storage/static/assets/codm.webp') }}" >
                        </div></center>
                        <div>
